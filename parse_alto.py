...
def parse_alto_file(file_name, result_dir = False, zip_file = False):
  ...
  if result_dir is False:
    ...
    final_file_name = result_dir + "/" + file_name[:-4].split("/")[-1]
  ...
  print(final_file_name + "is done.")

#A function to iterate over a directory.
def parse_alto_directory(source_dir, result_dir, zip_file = False):
  for file in glob.glob(source_dir + "/*.xml"):
    parse_alto_file(file, result_dir = result_dir, zip_file = zip_file)
